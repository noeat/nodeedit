...
waitmediator::waitmediator(int nodeid)
	:PureMVC::Mediator(id2mediatorname(nodeid)), nodeid_(nodeid)
{
	
}

std::vector<int> waitmediator::listNotificationInterests()
{
...
void waitmediator::handleNotification(PureMVC::INotification* notification)
{
	...
	assert(node != nullptr && node->type == NODETYPE::WAIT);
	...
	if (node->inputs[1].links.empty())
	{
		...
		ImGui::DragFloat(node->inputs[1].name, &node->inputs[1].value.float_, 0.03333, -1, 3600);
		ImGui::PopStyleVar(2);
		ImGui::PopItemWidth();
		ImGui::EndVertical();
	}
...
void waitmediator::onRegister()
{
	
}

void waitmediator::onRemove()
{
