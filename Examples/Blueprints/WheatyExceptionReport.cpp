...
//===========================================================
DWORD WINAPI WheatyExceptionReport::ThreadFunc(LPVOID p)
{
	PEXCEPTION_POINTERS pExceptionInfo = (PEXCEPTION_POINTERS)p;

	TCHAR *module_folder_name = new TCHAR[MAX_PATH];
	...
	TCHAR *crash_folder_path = new TCHAR[MAX_PATH];
	sprintf_s(crash_folder_path, MAX_PATH, "%s\\%s", module_folder_name, CrashFolder);
...
PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	if (pExceptionInfo->ExceptionRecord->ExceptionCode == STATUS_STACK_OVERFLOW)
	{
		...
		HANDLE hThread = CreateThread(NULL, 0, ThreadFunc, (LPVOID)pExceptionInfo, 0, nullptr); // 创建线程
