...
static ImFont* ImGui_LoadFont(ImFontAtlas& atlas, const char* name, float size, const ImVec2& displayOffset = ImVec2(0, 0))
{
    ...
    auto font = atlas.AddFontFromFileTTF(path.c_str(), size, &config, atlas.GetGlyphRangesChineseSimplifiedCommon());
    if (font)
...
# endif
    ...
    auto defaultFont = ImGui_LoadFont(fontAtlas, "simhei.ttf", 12.0f);//16.0f * 96.0f / 72.0f);
    fontAtlas.Build();
